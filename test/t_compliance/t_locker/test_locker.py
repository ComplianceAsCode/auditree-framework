...
def clean_temp():
    ...
    shutil.rmtree(str(Path(tempfile.gettempdir(), REPO_DIR)), ignore_errors=True)
...
class LockerTest(unittest.TestCase):
    ...
    def test_validate_raw_evidence(self, validation, check_mock, evidence_mock):
    ...
    def test_locker_as_expected(self):
        ...
        with Locker(name=REPO_DIR) as locker:
            ...
            self.assertTrue(
                locker.get_index_file(evidence_one).endswith(
                    f"/raw/test_category/{INDEX_FILE}"
                )
            )
            self.assertTrue(
                locker.get_index_file(evidence_two).endswith(
                    f"/raw/test_category/{INDEX_FILE}"
                )
            )
            self.assertTrue(
                locker.get_index_file(evidence_three).endswith(
                    f"/raw/test_category_two/{INDEX_FILE}"
                )
            )
            # Test abandoned
    ...
    def test_large_files(self):
        ...
        with Locker(name=REPO_DIR) as locker:
            large = RawEvidence("large.txt", "test_category", DAY, "Large evidence")
            large.set_content("X" * 10000)
            locker.add_evidence(large)
            small = RawEvidence("small.txt", "test_category", DAY, "Small evidence")
            small.set_content("X" * 10)
            locker.add_evidence(small)
            locker.checkin()
            self.assertEqual(
                locker.get_large_files(9999), {"raw/test_category/large.txt": 10000}
            )
    ...
    def test_add_partitioned_evidence(self):
        ...
        with Locker(name=REPO_DIR) as locker:
            ...
            self.assertEqual(meta["foo.json"]["partitions"][sobchak_hash], ["sobchak"])
            self.assertEqual(
    ...
    def test_get_partitioned_evidence(self):
        ...
        with Locker(name=REPO_DIR) as locker:
            ...
            self.assertEqual(json.loads(partitioned.get_partition(["meh"])), [])

    def test_get_unpartitioned_evidence(self):
        """Test that unpartitioned evidence is retrieved from the locker."""
        with Locker(name=REPO_DIR) as locker:
            evidence = RawEvidence("foo.json", "cat_foo", DAY, "Partitioned evidence")
            data = [
                {"fname": "walter", "lname": "sobchak"},
                {"fname": "jeff", "lname": "lebowski"},
            ]
    ...
    def test_abandoned_partitioned_evidence(self):
        ...
        with Locker(name=REPO_DIR) as locker:
            ...
            Path(locker.get_index_file(evidence)).write_text(json.dumps(abandoned_meta))
            locker.checkin()
            self.assertEqual(
                {"raw/cat_foo/f60fa4f29c_foo.json", "raw/cat_foo/59b70fc55f_foo.json"},
                locker.get_abandoned_evidences(),
            )
    ...
    def test_get_latest_commit_with_date_success(self):
        ...
        self.assertEqual(locker.get_latest_commit("raw/foo/foo.json", ev_date), "foo")
