...
class TestRawPartitionedEvidence(unittest.TestCase):
    ...
    def test_is_partitioned(self):
        ...
        evidence = RawEvidence("foo.json", "foo", partition={"fields": ["foo"]})
    ...
    def test_partition_keys_default_root(self):
        ...
        evidence = RawEvidence("foo.json", "foo", partition={"fields": ["foo", "bar"]})
    ...
    def test_partition_keys_root(self):
        ...
        evidence = RawEvidence(
            "foo.json", "foo", partition={"fields": ["foo", "bar"], "root": "root"}
        )
        evidence.set_content(
            json.dumps(
                {
    ...
    def test_partition_keys_nested_root(self):
        ...
        evidence = RawEvidence(
            "foo.json",
            "foo",
            partition={"fields": ["foo", "bar"], "root": "nested.root"},
        )
        evidence.set_content(
            json.dumps(
                {
            ...
            json.dumps(
                {
                    ...
                    "nested": {
                        "nested_stuff": "who cares?",
                        "root": [
                            {"dot_nota": {"foo": "FOO", "bar": "BAR"}, "baz": "BAZ"},
                            {"dot_nota": {"foo": "FOO", "bar": "BAR"}, "baz": "BLAH"},
                            {"dot_nota": {"foo": "FOO", "bar": "BOO"}, "baz": "BAZ"},
                        ],
                        "nested_other_stuff": "so what?",
                    },
                    "other_stuff": "so what?",
