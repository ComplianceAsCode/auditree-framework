...
from utilitarian.services import pagerduty
from utilitarian.services.github import GitHub
...
class GHEIssuesNotifier(_BaseMDNotifier):
    """
    Github Enterprise notifier class.

    Notifications are sent to Github Enterprise as repository issues.  This
    notifier is configurable via :class:`compliance.config.ComplianceConfig`.
    """

    def __init__(self, results, controls):
        """
        Construct and initialize the Github Enterprise notifier object.

        :param results: dictionary generated by
          :py:class:`compliance.runners.CheckMode` at the end of the execution.
        :param controls: the control descriptor that manages accreditations.
        """
        super(GHEIssuesNotifier, self).__init__(results, controls)
        self._config = get_config().get('notify.ghe_issues', {})
        self._github = GitHub(get_config().creds)

    def notify(self):
        """Send notifications to Github Enterprise as repository issues."""
        if not self._config:
    ...
    def _process_new_alerts(self, owner, repository, issues, message=None):
        issue_urls = {}
        for issue in issues:
            ghe_issue = self._find_ghe_issue(
                '/'.join([owner, repository]), issue['title']
            )
            if ghe_issue is None:
                body = issue['body']
                if message:
                    joined_msg = '\n'.join(message)
                    body = f'{joined_msg}\n\n{issue["body"]}'
                ghe_issue = self._github.add_issue(
                    owner,
                    repository,
                    issue['title'],
                    body,
                ...
                self._update_issue_labels(
                    owner, repository, ghe_issue, issue['labels']
                )
                self._github.add_issue_comment(
                    owner, repository, ghe_issue['number'], issue['body']
                )
            issue_urls[ghe_issue['id']] = ghe_issue['url']
        return issue_urls
    ...
    def _find_ghe_issue(self, repo, title):
        ghe_issues = self._github.search_issues(
            f'{title} type:issue in:title is:open repo:{repo}'
        )
        found = None
        for issue in ghe_issues:
    ...
    def notify(self):
        ...
        for accreditation, desc in messages:
            ...
            for test_id, test_desc, msg in (failed + errored):
                ...
                existing_alert = next(
                    (a for a in alerts if a['test'] == test_id), None
                )
                ...
                if existing_alert:
                    self._update_alert(
                        test_id, test_desc, msg, accreditation, details
                    )
                else:
                    self._trigger_alert(
                        test_id, test_desc, msg, accreditation, details
